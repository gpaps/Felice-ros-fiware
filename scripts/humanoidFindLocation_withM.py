...
# obUids = p.loadMJCF("./nhumanoid.xml")
# humanoid = obUids[1]

# obUid = p.loadURDF("/my_humanoid_v2.urdf", [-.01, -.01, 1.3],
...
def talker():
    ...
    rospy.init_node('talkerGpaps', anonymous=True)
    rate = rospy.Rate(1)  # 10hz
    while not rospy.is_shutdown():
        string_ = 'ROS Knowledge Pybullet Was here'

        p.getBasePositionAndOrientation(end_effectorId)
...
def pos_ornt2ros(*args):
    return p.getBasePositionAndOrientation(args)
...
while 1:
    ...
    if poscount == 10:  # This finds the first 10 green positions, but..... we may return the first one and stop!
        print(greenPoses)
